...
A successful run ends with `"status": "passed"`, proves the installed package
matches this checkout, and includes a scalar discovery manifest with stable
root-relative file/test IDs and source locations.

## Known boundary

This is a one-test discovery-to-execution bridge, not the Drove runner. It does
not yet replace PHPUnit's suite runner, replay child events into Pest reporters,
support datasets or nested scopes, or classify setup failures. Generated PHPUnit
identities remain execution evidence and are not part of the scalar manifest.
